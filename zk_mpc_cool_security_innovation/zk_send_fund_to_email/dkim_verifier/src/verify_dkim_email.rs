...
pub fn verify_email(
    ...
    verify_signature_helper(&verifying_key, hasher_headers, &signature).to_dkim_verifier_err()?;
...
fn extract_body(raw_email: &str) -> Result<String, DkimError> {
    use mailparse::parse_mail;
    let parsed = parse_mail(raw_email.as_bytes()).to_dkim_verifier_err()?;
    let parsed_body = parsed.get_body().to_dkim_verifier_err();
    println!("parsed_body is {:?}", parsed_body);

    parsed_body
}

fn verify_signature_helper(
